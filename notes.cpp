...
short* glisstonegen(unsigned int *freq, unsigned int duty, double volume, unsigned int *len,int durr){

    int durr_samp = (double)durr/60.0*sfreq;


    short *buf = (short*)malloc(durr_samp*2+4);

    volume /=4;

    int flen=0;
    for(flen=0;freq[flen]!=0;++flen){
    };

    for(int f=0;f<flen;++f){
        for(int i=durr_samp/(flen+1)*f;i<(durr_samp/(flen+1)*(f+1));++i){
            if(((i-(durr_samp/(flen+1)*f))%(sfreq/freq[f]))<(sfreq/freq[f]*duty/100)){
                buf[i]=32767 * volume;
